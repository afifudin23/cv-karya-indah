...
import {
    FiCalendar, FiUser, FiMapPin, FiFileText,
    FiEye, FiTarget, FiShield, FiArrowLeft, FiArrowRight,
    FiPenTool, FiGrid, FiHome, FiTool,
    FiPhone, FiMail, FiBriefcase, FiTag,
} from "react-icons/fi";
import ImageViewer from "../ui/ImageViewer";
import { Reveal, RevealStagger } from "../ui/Reveal";

const profileHighlights = [
    { icon: FiCalendar, label: "Berdiri Resmi",   value: "11 Desember 1992", detail: "Lebih dari tiga dekade melayani kebutuhan branding dan konstruksi visual." },
    { icon: FiMapPin,   label: "Lokasi",           value: "Kota Tegal",       detail: "Basis operasional yang mendukung layanan regional dan proyek skala luas." },
];

const serviceCards = [
    { icon: FiPenTool, title: "Exterior Goods",   desc: "Signage fasad, billboard, neon box, dan huruf timbul untuk identitas visual luar ruangan." },
    { icon: FiGrid,    title: "Interior Goods",   desc: "Wall grafis, display point-of-sale, dan elemen dekorasi interior branded untuk ruang bisnis." },
    { icon: FiHome,    title: "Digital Printing", desc: "Cetak banner, spanduk, stiker, dan berbagai media cetak format besar resolusi tinggi." },
    { icon: FiTool,    title: "Konstruksi",        desc: "Konstruksi rangka, pemasangan instalasi, hingga finishing proyek visual skala besar." },
];
...
const legalityItems = [
    {
        id: "01", image: "/legalitas1.jpeg", eyebrow: "OSS RBA",
        title: "Sertifikat Standar",
        subtitle: "Perizinan berusaha berbasis risiko",
        summary: "Dokumen utama OSS yang menegaskan identitas usaha CV Karya Indah, berikut NIB, alamat usaha, dan klasifikasi kegiatan utama.",
        meta: ["NIB 9120207830213", "Terbit 20 Agustus 2024", "KBLI 41019"],
    },
    {
        id: "02", image: "/legalitas2.jpeg", eyebrow: "Perpajakan",
        title: "Surat Keterangan Terdaftar Pajak",
        subtitle: "Administrasi NPWP badan usaha",
        summary: "Dokumen perpajakan yang memuat NPWP badan, status administrasi, serta rujukan kantor pajak terdaftar untuk perusahaan.",
        meta: ["NPWP 01.527.761.9-501.000", "KPP Pratama Tegal", "8 Agustus 2024"],
    },
    {
        id: "03", image: "/legalitas3.jpeg", eyebrow: "Lampiran OSS",
        title: "Lampiran KBLI dan Persyaratan",
        subtitle: "Ruang lingkup izin usaha",
        summary: "Lampiran yang merangkum kode KBLI, klasifikasi risiko, serta persyaratan dan kewajiban usaha terkait sertifikat standar.",
...
export default function ProfileSection({ companyData }) {
    ...
    const highlights = [
        ...profileHighlights,
        { icon: FiUser,     label: "Direktur",  value: companyData.director,                    detail: "Kepemimpinan yang menjaga kualitas produksi, layanan, dan relasi klien." },
        { icon: FiFileText, label: "Legalitas", value: `${companyData.legalities.length} Dokumen Utama`, detail: "Dokumen inti perusahaan tersedia untuk mendukung kerja sama profesional." },
    ];
            ...
            <div className="relative flex items-center justify-center overflow-hidden pt-[var(--header-height)] min-h-[100svh]">
                ...
                <div className="relative z-10 mx-auto max-w-3xl px-6 py-20 text-center text-white sm:px-12">
                    <h1 className="font-bold text-[2.4rem] leading-tight sm:text-[3.5rem]">
                        Mitra Visual Bisnis<br />Anda Sejak 1992
                    </h1>
                    <p className="mx-auto mt-5 max-w-2xl text-[1rem] leading-relaxed text-white/80">
                        CV Karya Indah adalah perusahaan periklanan dan konstruksi visual yang telah melayani
                        berbagai klien korporat di seluruh Indonesia dengan standar kualitas tinggi.
                    </p>
                ...
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="grid gap-14 lg:grid-cols-2 lg:items-stretch">
                        ...
                        <Reveal as="div" className="flex flex-col justify-center">
                            ...
                            <h2 className="mb-6 font-bold text-[2rem] sm:text-[2.5rem] leading-tight text-gray-900">
                                Siapa Kami
                            </h2>
                        ...
                        <RevealStagger as="div" className="grid grid-cols-2 grid-rows-2 gap-4 h-full" stagger={70}>
                            {serviceCards.map((card) => {
                                ...
                                return (
                                    <div
                                        key={card.title}
                                        className="flex items-center gap-5 rounded-2xl bg-[var(--color-primary-50)] px-7 transition duration-300 hover:-translate-y-1 h-full"
                                    >
                                        <div className="shrink-0">
                                            <Icon className="text-[2.2rem] text-[var(--color-primary-600)] stroke-[1.4]" />
                                        </div>
                                        <h3 className="font-bold text-[1.05rem] text-gray-900 leading-snug">{card.title}</h3>
                                    </div>
                                );
                            })}
                        </RevealStagger>
                ...
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <RevealStagger as="div" className="grid gap-10 md:grid-cols-2" stagger={80}>
                        ...
                        <div className="flex flex-col items-center text-center px-4">
                            ...
                            />
                            <p className="text-[0.82rem] font-semibold text-gray-500 mb-1">
                                Visi Perusahaan
                            </p>
                            ...
                            <p className="text-[0.97rem] leading-7 text-gray-600 max-w-md">
                                {companyData.vision}
                            </p>
                        </div>
                        ...
                        <RevealStagger as="div" stagger={50}>
                            ...
                            <div className="space-y-3">
                                {companyRows.map((item) => {
                                    const Icon = item.icon;
                                    return (
                                        <div key={item.label} className="flex items-start gap-4 rounded-2xl bg-[var(--color-primary-50)] px-5 py-4">
                            ...
                            <RevealStagger as="div" stagger={60}>
                                ...
                                <div className="space-y-3">
                                    {companyData.legalities.map((item, index) => (
                                        <div key={item.label} className="flex items-start gap-4 rounded-2xl border border-gray-100 bg-white px-5 py-5 shadow-sm">
                            ...
                            <Reveal as="div" delay={80}>
                                <div className="rounded-2xl bg-[var(--color-primary-600)] p-7 text-white">
                                    ...
                                    <p className="text-[0.9rem] leading-7 text-white/80">
                                        CV Karya Indah menerapkan standar K3 secara disiplin pada setiap tahapan produksi, pengiriman, hingga instalasi di lapangan.
                                    </p>
                                </div>
                            </Reveal>
                        </div>
...
function LegalitiesShowcase({ items, onOpenImage }) {
    ...
    const lightboxItems = items.map((item) => ({
        src: item.image, alt: item.title, title: item.title, eyebrow: item.eyebrow,
    }));
